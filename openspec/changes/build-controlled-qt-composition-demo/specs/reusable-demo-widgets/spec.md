...
系统 SHALL 将 `Calculator`、`CalculationHistory`、`CalculationStats`、`Clock` 和 `NotePad` 实现为可嵌入、非顶层的 C++ QWidget 类，并在 `LegacyToolboxWindow` 和动态 Renderer 中使用相同的类，不得复制或改造它们的内部控件树。

#### Scenario: 固定版与动态版使用相同实现
- **WHEN** 同一组件类型同时显示在传统窗口和动态 Surface 中
- **THEN** 两个实例均由同一个已注册 C++ QWidget 类构造
